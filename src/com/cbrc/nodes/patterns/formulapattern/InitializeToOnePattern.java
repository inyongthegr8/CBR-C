...
public class InitializeToOnePattern extends FormulaPatternNode {
			...
			if (node instanceof AssignmentStatementNode) {
				...
				if (expression.getChildNodes().size() == 1) {
					...
					if (nodeFirst instanceof IntegerLiteralToken) {
						if (((IntegerLiteralToken) nodeFirst).getToken().equals("1")) {
							return true;
						}
					}
				}
	...
	private final String ERROR_2 = "Code does not does not initialize the variable to 1. To initialize a variable to 1, assign 1 to it.";
